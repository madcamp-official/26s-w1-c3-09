...
\subsubsection*{entity/ --- DB 테이블과 1:1 매핑 12개 (원본: docs/KJH/db-schema.sql)}
\begin{Verbatim}[frame=single, fontsize=\small]
...
\begin{enumerate}
  \item \textbf{MySQL 준비}: 로컬 MySQL 8.0에서 \texttt{roblox\_rec} 데이터베이스 생성 후
        \texttt{docs/KJH/db-schema.sql}을 실행 (DBeaver에서 열어서 전체 실행).
...
시행착오 문서(왜 이런 구조가 됐는지), \texttt{docs/KJH/의사결정-기록.md}(모든 결정의 근거).
